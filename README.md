...
- **Organized by life domain, Google-Drive-style.** Pages live under typed top-level folders the agent picks up itself — `business/`, `trading/`, `goals/`, `dream/`, etc. — with slugs like `business/acme` or `trading/polymarket-2026q2`. *Why:* it should feel like a tidy Drive, not a dumped chat log. A human opening `pages/` should be able to skim a directory listing and know what's in there; an agent should be able to `get_page("business/acme")` deterministically instead of guessing search terms. Combined with disk-first markdown and lineage on every write, that gives you the Drive feel — organized, browsable, and traceable to exactly which agent put each file there.
